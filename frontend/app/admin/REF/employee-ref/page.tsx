...
import { Plus, Search, Calendar, Phone, User, Edit, DollarSign, ChevronLeft, ChevronRight } from "lucide-react"
import { settingsApi } from '@/lib/settingsApi'
import { useRouter } from "next/navigation"
import { format } from "date-fns"
import Link from "next/link"

export default function EmployeeRefListPage() {
  ...
  const fetchRefPatients = async () => {
    setIsLoading(true)
    try {
      const response = await settingsApi.getEmployeeRefPatients(currentPage, pageSize)
      setRefPatients(response.data || [])
      setTotalPages(response.totalPages || 1)
      setTotalRecords(response.total || 0)
    } catch (error) {
      console.error('Failed to fetch employee ref patients:', error)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const filteredPatients = refPatients.filter(patient =>
    ...
    `${patient.referrer_first_name} ${patient.referrer_last_name}`.toLowerCase().includes(searchTerm.toLowerCase())
  )
  ...
  return (
    <div className="p-6 space-y-4 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Employee REF Patients</h1>
          <p className="text-gray-600">View and manage patients referred by employees</p>
        </div>
        <Button
          className="bg-red-600 hover:bg-red-700"
          onClick={() => router.push("/admin/REF/employee-ref/add")}
        >
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <div className="relative w-64">
              ...
              <Input
                placeholder="Search patients or employees..."
                className="pl-8"
                value={searchTerm}
            ...
            <table className="w-full">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Referred By (Employee)</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="px-4 py-8 text-center text-gray-500">
                      No Employee REF patients found
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={patient.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-700">
                        <span className="px-2 py-1 bg-green-50 text-green-700 rounded-md text-xs font-medium border border-green-100">
                          {`${patient.referrer_first_name} ${patient.referrer_last_name}`}
                        </span>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-700">
                        <div className="flex items-center">
