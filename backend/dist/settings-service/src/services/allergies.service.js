...
let AllergiesService = class AllergiesService {
    ...
    async getAllergiesOptions(allergiesId) {
        try {
            const result = await this.pool.query('SELECT * FROM allergies_options WHERE allergies_id = $1 ORDER BY id', [allergiesId]);
            return result.rows;
        }
        catch (error) {
            console.error('Database error:', error);
    ...
    async savePatientAllergies(data, user) {
        try {
      ...
      `);
            ...
            if (!allergies_option_id && notes !== undefined) {
                const updateResult = await this.pool.query('UPDATE patient_allergies SET notes = $1 WHERE patient_id = $2 AND location_id = $3', [notes, patient_id, location_id]);
                if (updateResult.rowCount === 0) {
                    await this.pool.query('INSERT INTO patient_allergies (patient_id, allergies_id, allergies_option_id, category_title, option_title, location_id, notes) VALUES ($1, $2, $3, $4, $5, $6, $7)', [patient_id, 1, 0, 'General', 'Notes Placeholder', location_id, notes]);
                }
                return { success: true, message: 'Notes updated' };
            }
            let numericAllergiesId = allergies_id;
            if (typeof allergies_id === 'string' && isNaN(Number(allergies_id))) {
                const allergiesResult = await this.pool.query('SELECT id FROM allergies WHERE title ILIKE $1', [allergies_id]);
            ...
            const existingRecord = await this.pool.query('SELECT id FROM patient_allergies WHERE patient_id = $1 AND allergies_option_id = $2 AND location_id = $3', [patient_id, allergies_option_id, location_id]);
            if (existingRecord.rows.length > 0) {
                if (notes !== undefined) {
                    await this.pool.query('UPDATE patient_allergies SET notes = $1 WHERE patient_id = $2 AND location_id = $3', [notes, patient_id, location_id]);
                }
                return { message: 'Record already exists' };
            }
            let finalNotes = notes;
            if (finalNotes === undefined || finalNotes === null || finalNotes === '') {
                const existingNoteResult = await this.pool.query('SELECT notes FROM patient_allergies WHERE patient_id = $1 AND notes IS NOT NULL AND notes != \'\' LIMIT 1', [patient_id]);
                if (existingNoteResult.rows.length > 0) {
                    finalNotes = existingNoteResult.rows[0].notes;
                }
            }
            const result = await this.pool.query('INSERT INTO patient_allergies (patient_id, allergies_id, allergies_option_id, category_title, option_title, location_id, notes) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id', [patient_id, numericAllergiesId, allergies_option_id, category_title, option_title, location_id, finalNotes]);
    ...
    async getPatientAllergies(patientId, locationId, user) {
        try {
        ...
        ADD COLUMN IF NOT EXISTS notes TEXT
      `);
            const result = await this.pool.query(`SELECT pa.*, 
         ...
         ORDER BY COALESCE(a.title, pa.category_title), COALESCE(ao.title, pa.option_title)`, [numericPatientId]);
    ...
    async deletePatientAllergies(data, user) {
        try {
            ...
            await this.pool.query('DELETE FROM patient_allergies WHERE patient_id = $1 AND allergies_option_id = $2 AND location_id = $3', [patient_id, allergies_option_id, location_id]);
            return { success: true };
        }
        catch (error) {
            console.error('Database error:', error);
